...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (awsAccessKeyId && awsSecretAccessKey && awsRegion && cognitoUserPoolId) {
      console.log('Deleting user from Cognito:', email);
      
      const service = "cognito-idp";
      const host = `${service}.${awsRegion}.amazonaws.com`;
      const target = "AWSCognitoIdentityProviderService.AdminDeleteUser";
      ...
      if (!cognitoResponse.ok) {
        ...
        console.error('Cognito user deletion error:', errorText);
        
        // Try fallback with ListUsers to get actual Username
        try {
          ...
          if (listResp.ok) {
            ...
            if (username) {
              ...
              // Retry AdminDeleteUser with resolved Username
              ...
              if (!retryResp.ok) {
                ...
                console.error('Cognito retry deletion error:', retryText);
              } else {
                console.log('User deleted from Cognito successfully (after resolving Username)');
              }
            }
          }
        } catch (listErr) {
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        message: "User deleted from both Supabase and Cognito" 
      }),
